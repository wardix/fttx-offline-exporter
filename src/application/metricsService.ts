...
export async function pollMetricsBackground(): Promise<void> {
  ...
  try {
    ...
    for (const alert of alertList) {
      ...
      output += `fttx_subscriber_offline_status{subscriber_id="${subId}",subscriber_name="${subName}",circuit_id="${circuitId}",status="${status}",cause="${cause}",offline_time="${offlineTime}",operator="fbstar",fttx="yes"} 1\n`
    }
